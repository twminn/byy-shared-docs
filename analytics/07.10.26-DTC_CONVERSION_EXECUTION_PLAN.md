...
| Soft funnel metrics | Browse / Engage / Purchase (see §3.1) |

### 3.1 Soft funnel metrics (Browse / Engage / Purchase)

Paid traffic is measured in three tiers. **Purchase stays the primary business KPI**; Engage is the soft conversion for middle-funnel ads; Browse is reporting-only (not a Meta bid event).

| Tier | Meaning | dataLayer | GA4 | Meta |
|------|---------|-----------|-----|------|
| **Browse** | Meaningful on-site activity (not a bounce) | `soft_browse` once/session | Reporting only — **not** a key event | **Do not** send |
| **Engage** | Soft CTA: tour, YouTube play, DMS subscribe | `soft_engage` + `engage_type` | Key event | `fbq('trackCustom', 'Engage', …)` |
| **Purchase** | Stripe pay | `purchase` (existing) | Key event | Purchase (existing) |

**Browse fires when** any of: `scroll_milestone` ≥ 25, `engaged_time` ≥ 15s, or `section_view`.

**Engage types:** `tour` (tour CTA / `tour_start`), `youtube` (video play only), `dms` (when S5 ships).

**Scope now:** GA4 + Meta. TikTok Engage deferred until cold TikTok spend resumes (S7).

**Frontend:** `byy-tracker.js` pushes `soft_browse` / `soft_engage`.  
**Ads:** Optimize middle-funnel campaigns on Meta custom event `Engage`; keep Purchase for bottom-funnel.
...
| N3 | Install Microsoft Clarity via GTM | Analytics | GTM `GTM-K9PVK9ZF` | **Done** 2026-07-10 — v15, project `xkbhm740u6` |
| N6 | Soft funnel Browse/Engage metrics (GA4 + Meta) | Analytics | Tracker + GTM + GA4 key events | **Done** 2026-07-10 — GTM v16; GA4 key `soft_engage`/`tour_start`; LP [PR #11](https://github.com/twminn/byy-landing-pages/pull/11) |
...
| # | Item | Owner | Notes |
|---|------|-------|-------|
| S1 | Mid-page YouTube on `new` + main | Frontend | **Done** 2026-07-10 — [PR #9](https://github.com/twminn/byy-landing-pages/pull/9) `new`, [PR #10](https://github.com/twminn/byy-landing-pages/pull/10) main |
