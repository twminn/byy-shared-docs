# Annotation Setup — Frontend Team

This guide walks the Frontend team through adding deploy annotations to the BYY
marketing analytics dashboard. Once configured, every production deploy of the
frontend site (`bestyearyet.com`) will appear as an **emerald ▼** marker on all
dashboard time-series charts, letting the whole organization correlate traffic
and revenue shifts with frontend releases.
...
easy to miss that a landing page redesign or copy change caused a conversion
shift.
...
`byy-marketing-analytics` repository into the root of your frontend repo so the
path is:
...
If your workflow already configures AWS credentials for deployment (e.g. S3
sync, CloudFront invalidation), attach this policy to the same role. If not,
create a dedicated IAM user or role and proceed to Step 3.

## Step 3 — GitHub Secrets

Add the following secrets to your frontend repository (Settings → Secrets and
variables → Actions):
...
For releases that are better described as a site update or bug fix, override
the `category` input:
...
- name: Annotate deployment
  ...
  with:
    source: byy-frontend
    category: site_update        # or feature_release, bug_fix, etc.
```
...
3. Confirm that a new row appears with source `byy-frontend`.
4. Check any time-series chart — you should see an **emerald ▼** marker on
